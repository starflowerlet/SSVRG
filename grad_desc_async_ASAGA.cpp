...
chrono::steady_clock::time_point start_SAGA = chrono::steady_clock::now();
    ...
    for(size_t i = 0; i < inner_iters; i ++) {
        ...
        if(is_store_result) {
            if(!((iter_no + 1) % (3 * N))) {
				...
				chrono::duration<double>time_used = chrono::duration_cast<chrono::duration<double>>(end - start_SAGA);
