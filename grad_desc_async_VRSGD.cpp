...
chrono::steady_clock::time_point start_VRSGD = chrono::steady_clock::now();

std::atomic<int> VRSGD_counter(0);
void grad_desc_async_sparse::VRSGD_Inner_Loop(double* X, double* Y, size_t* Jc
...
std::vector<double>* grad_desc_async_sparse::VRSGD_Async(double* X
    ...
    if(is_store_result) {
		...
		chrono::duration<double>time_used = chrono::duration_cast<chrono::duration<double>>(end - start_VRSGD);
    ...
    for(size_t i = 0 ; i < iteration_no; i ++) {
        ...
        for(size_t k = 1; k <= thread_no; k ++) {
            ...
            thread_pool.push_back(std::thread(VRSGD_Inner_Loop, X, Y, Jc, Ir, N
                , x, aver_x, model, m, inner_iters, step_size, reweight_diag
